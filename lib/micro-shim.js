/*
 Problem with proton on node v6. In the meantime, this copy of micro decouples them
 so clique can define a webapp.
...
function handlePublicTemplates (ext, callback) {
    this.get(/^((?:\/[\w_-]+)+)\/$/, function (request, response, path) {
        return response.redirect(path, true);
    });

    this.get(/^(?:(\/)|((?:\/[\w_-]+)+))$/, function (request, response, index, path) {
        if (path && path === '/index') {
...
//                // TODO - perhaps allow fall through to next route?
            ...
            response.addToBody(typeof body === 'undefined' ? '' : body);
            return response._response;
        }
    }
